...
class AdminModel extends Model {
    ...
    public function initPagesTable() {
        try {
            $this->db->exec("
    ...
    public function addPage($data) {
        $slug = trim(preg_replace('~[^\p{L}\p{N}\p{M}]+~u', '-', mb_strtolower($data['title'], 'UTF-8')), '-');
        if (empty($slug)) $slug = 'page-' . time();
        
        // Ensure unique slug
        $stmtCheck = $this->db->prepare("SELECT id FROM pages WHERE slug = ?");
    ...
    public function ensureUniqueSlug($slug, $exclude_id = null) {
        $slug = trim($slug);
        if (empty($slug)) {
            $slug = 'post-' . time();
        }
        ...
        while (true) {
            if ($exclude_id) {
                $stmt = $this->db->prepare("SELECT COUNT(*) FROM posts WHERE slug = :slug AND id != :id");
                $stmt->execute(['slug' => $slug, 'id' => $exclude_id]);
            } else {
    ...
    public function createPost($data) {
        ...
        if (empty($data['category_id'])) {
            $first_cat = $this->db->query("SELECT id FROM categories ORDER BY id ASC LIMIT 1")->fetchColumn();
            if ($first_cat) {
                $data['category_id'] = $first_cat;
                $category_ids = [$first_cat];
            }
        }
    ...
    public function updatePost($data) {
        ...
        if (!empty($data['id'])) {
            $data['slug'] = $this->ensureUniqueSlug($data['slug'] ?? $data['title'], $data['id']);
        }
    ...
    public function bulkDeletePosts($ids) {
        ...
        $res = $this->db->query("DELETE FROM posts WHERE id IN ($in)");
        if ($res) {
            \App\Helpers\Cache::clear();
        }
        return $res;
    ...
    public function deleteAuthor($id) {
        ...
        // Find default/fallback author (Admin or ID 1)
        $adminAuthor = $this->db->query("SELECT id FROM authors WHERE name = 'Admin' AND id != {$id} LIMIT 1")->fetchColumn();
        if (!$adminAuthor) {
            $adminAuthor = $this->db->query("SELECT id FROM authors WHERE id != {$id} ORDER BY id ASC LIMIT 1")->fetchColumn();
        }
        
        if ($adminAuthor) {
            // Reassign posts in post_authors
            $stmt_pa = $this->db->prepare("UPDATE IGNORE post_authors SET author_id = :admin_id WHERE author_id = :id");
            $stmt_pa->execute(['admin_id' => $adminAuthor, 'id' => $id]);
            
            // Delete leftover post_authors records for this author
            $stmt_del_pa = $this->db->prepare("DELETE FROM post_authors WHERE author_id = :id");
            $stmt_del_pa->execute(['id' => $id]);
            
            // Reassign string author name in posts if matching
            $deletedAuthor = $this->getAuthorById($id);
            if ($deletedAuthor) {
                $adminName = $this->db->query("SELECT name FROM authors WHERE id = {$adminAuthor}")->fetchColumn() ?: 'Admin';
                $stmt_p = $this->db->prepare("UPDATE posts SET author = :admin_name WHERE author = :author_name");
                $stmt_p->execute(['admin_name' => $adminName, 'author_name' => $deletedAuthor['name']]);
            }
        }
    ...
    public function getComments() {
        $stmt = $this->db->query("SELECT c.*, p.title as post_title, p.slug as post_slug FROM comments c JOIN posts p ON c.post_id = p.id ORDER BY c.created_at DESC");
        return $stmt->fetchAll();
    }

    public function getCommentsByPostId($post_id) {
        $stmt = $this->db->prepare("SELECT * FROM comments WHERE post_id = :post_id AND status = 'approved' ORDER BY created_at ASC");
        $stmt->execute(['post_id' => $post_id]);
        return $stmt->fetchAll();
    }
    ...
    // Team Management
    private function initTeamMembersTable() {
        try {
            $this->db->query("CREATE TABLE IF NOT EXISTS team_members (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                designation VARCHAR(255) NOT NULL,
                image VARCHAR(550) NOT NULL,
                facebook_link VARCHAR(550) NULL,
                twitter_link VARCHAR(550) NULL,
                linkedin_link VARCHAR(550) NULL,
                order_index INT DEFAULT 0,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

            $this->db->query("SELECT linkedin_link FROM team_members LIMIT 1");
        } catch (\PDOException $e) {
            try {
                $this->db->query("ALTER TABLE team_members ADD COLUMN linkedin_link VARCHAR(550) NULL");
            } catch (\PDOException $e2) {}
        }
    }

    public function getTeamMembers() {
        $this->initTeamMembersTable();
        $stmt = $this->db->query("SELECT * FROM team_members ORDER BY order_index ASC, id ASC");
    ...
    public function addTeamMember($data) {
        $this->initTeamMembersTable();
        $stmt = $this->db->prepare("INSERT INTO team_members (name, designation, image, facebook_link, twitter_link, linkedin_link, order_index) 
                                    VALUES (:name, :designation, :image, :facebook_link, :twitter_link, :linkedin_link, :order_index)");
        return $stmt->execute([
            'name' => $data['name'],
    ...
    public function updateTeamMember($id, $data) {
        ...
        $stmt = $this->db->prepare("UPDATE team_members SET name = :name, designation = :designation, image = :image, 
                                    facebook_link = :facebook_link, twitter_link = :twitter_link, linkedin_link = :linkedin_link, order_index = :order_index 
                                    WHERE id = :id");
        return $stmt->execute([
            'id' => $id,
            'name' => $data['name'],
    ...
    public function deleteMenu($id) {
        // Delete child menu items if parent is deleted to prevent orphan menu items
        $stmt_child = $this->db->prepare("DELETE FROM menus WHERE parent_id = :id");
    ...
    // Sub Menu Management
    private function initSubMenusTable() {
        try {
            $this->db->query("CREATE TABLE IF NOT EXISTS sub_menus (
                id INT AUTO_INCREMENT PRIMARY KEY,
                title VARCHAR(255) NOT NULL,
                url VARCHAR(255) NOT NULL,
                order_index INT DEFAULT 0,
            ...
            // Table creation log/silence if exists
        }
    }

    public function getSubMenus() {
        $this->initSubMenusTable();
    ...
    private function initReviewsTable() {
        ...
        try {
            $this->db->query("SELECT status FROM reviews LIMIT 1");
        } catch (\PDOException $e) {
            try {
                $this->db->query("ALTER TABLE reviews ADD COLUMN status VARCHAR(20) DEFAULT 'active'");
            } catch (\PDOException $e2) {}
        }

        // Seed with 6 reviews if empty
        ...
        if ($count == 0) {
            ...
            $stmt = $this->db->prepare("INSERT INTO reviews (name, designation, image, rating, review_text, status, order_index) VALUES (:name, :designation, :image, :rating, :review_text, :status, :order_index)");
    ...
    public function getReviews($only_active = false) {
    ...
    public function updateReview($id, $data) {
        ...
        $stmt = $this->db->prepare("UPDATE reviews SET name = :name, designation = :designation, image = :image, rating = :rating, review_text = :review_text, status = :status, order_index = :order_index WHERE id = :id");
        return $stmt->execute([
            'id' => $id,
            'name' => $data['name'],
            'designation' => $data['designation'] ?? null,
    ...
    private function initJoinCardsTable() {
        $this->db->query("CREATE TABLE IF NOT EXISTS join_cards (
            ...
            button_url VARCHAR(550) NULL,
            order_index INT DEFAULT 0,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

        try {
            $this->db->query("SELECT button_text FROM join_cards LIMIT 1");
        } catch (\PDOException $e) {
            try {
                $this->db->query("ALTER TABLE join_cards ADD COLUMN button_text VARCHAR(100) NULL, ADD COLUMN button_url VARCHAR(550) NULL");
            } catch (\PDOException $e2) {}
        }
        ...
        if ($count == 0) {
            $default_cards = [
                ...
                [
                    ...
                    'button_text' => 'পোর্টফোলিও পাঠান',
                    'button_url' => '/contact',
                    'order_index' => 3
                ]
            ];
            $stmt = $this->db->prepare("INSERT INTO join_cards (title, icon, description, button_text, button_url, order_index) VALUES (:title, :icon, :description, :button_text, :button_url, :order_index)");
    ...
    public function updateJoinCard($id, $data) {
        $this->initJoinCardsTable();
        $stmt = $this->db->prepare("UPDATE join_cards SET title = :title, icon = :icon, description = :description, button_text = :button_text, button_url = :button_url, order_index = :order_index WHERE id = :id");
        return $stmt->execute([
            'id' => $id,
            'title' => $data['title'],
            'icon' => $data['icon'],
    ...
    public function getFilteredUserQuestionsCount($status = 'answered', $category_slug = null, $search = null, $user_id = null, $only_bookmarks = false) {
        $sql = "SELECT COUNT(*) FROM user_questions q LEFT JOIN question_categories c ON q.category_id = c.id";
        $where = [];
        $params = [];

        if ($only_bookmarks && $user_id) {
            $sql .= " INNER JOIN user_question_bookmarks b ON q.id = b.question_id AND b.user_id = :b_user_id";
    ...
    public function getFilteredUserQuestions($limit = 10, $offset = 0, $status = 'answered', $category_slug = null, $search = null, $user_id = null, $only_bookmarks = false) {
        $sql = "SELECT q.*, c.name as category_name, c.slug as category_slug FROM user_questions q LEFT JOIN question_categories c ON q.category_id = c.id";
        $where = [];
        $params = [];
