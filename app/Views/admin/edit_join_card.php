    ...
    <form action="<?= URLROOT ?>/admin/edit_join_card/<?= $data['card']['id'] ?>" method="POST">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group mb-4">
                ...
                <input type="text" name="button_text" class="form-control" value="<?= htmlspecialchars($data['card']['button_text'] ?? '') ?>" placeholder="e.g. যুক্ত হন / যোগাযোগ করুন">
            </div>
            <div class="form-group mb-4">
                <label>Button URL (বাটন লিংক)</label>
                <input type="text" name="button_url" class="form-control" value="<?= htmlspecialchars($data['card']['button_url'] ?? '') ?>" placeholder="e.g. /contact or https://forms.gle/...">
            </div>
        </div>
