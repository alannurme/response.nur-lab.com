...
<div class="admin-card" style="max-width: 800px; margin: 0 auto;">
    <div class="justify-content-between d-flex align-items-center mb-5">
        <h3 class="m-0">Edit Team Member: <?= $data['member']['name'] ?></h3>
        <a href="<?= URLROOT ?>/admin/team" class="btn btn-visit-site">Back to List</a>
    </div>

    <form action="<?= URLROOT ?>/admin/edit_team_member/<?= $data['member']['id'] ?>" method="POST">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px;">
            <div class="form-group mb-4">
                <label>Full Name</label>
                <input type="text" name="name" value="<?= $data['member']['name'] ?>" class="form-control" required>
            </div>
            <div class="form-group mb-4">
                <label>Designation</label>
                <input type="text" name="designation" value="<?= $data['member']['designation'] ?>" class="form-control" required>
            </div>
        </div>
        ...
        <div class="form-group mb-4">
            ...
            <div style="display: flex; gap: 20px; align-items: flex-start;">
                <div style="width: 120px; height: 120px; border-radius: 15px; overflow: hidden; border: 1px solid var(--primary); background: #f8fafc;">
                    <img id="member_preview" src="<?=(strpos($data['member']['image'], 'http') === 0 || strpos($data['member']['image'], '/') === 0) ? $data['member']['image'] : URLROOT . '/public/img/' . $data['member']['image']?>" loading="lazy" decoding="async" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
                <div style="flex: 1;">
                    <input type="text" id="member_image_path" name="image" value="<?= $data['member']['image'] ?>" class="form-control" required readonly>
                    <div style="display: flex; gap: 10px; margin-top: 10px;">
                        ...
                        <button type="button" id="crop_btn" class="btn btn-warning w-100" onclick="startCropper()" style="background: #eab308; color: #fff; border: none; font-weight: 600; display: <?= !empty($data['member']['image']) ? 'inline-block' : 'none' ?>;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
            <div class="form-group mb-4">
                ...
                <input type="url" name="facebook_link" value="<?= htmlspecialchars($data['member']['facebook_link'] ?? '') ?>" class="form-control">
            </div>
            <div class="form-group mb-4">
                <label><i class="fab fa-twitter" style="color: #1da1f2;"></i> Twitter / X URL</label>
                <input type="url" name="twitter_link" value="<?= htmlspecialchars($data['member']['twitter_link'] ?? '') ?>" class="form-control">
            </div>
            <div class="form-group mb-4">
                <label><i class="fab fa-linkedin" style="color: #0a66c2;"></i> LinkedIn URL</label>
                <input type="url" name="linkedin_link" value="<?= htmlspecialchars($data['member']['linkedin_link'] ?? '') ?>" class="form-control">
            </div>
        </div>

        <div class="form-group mb-5">
            <label>Display Order</label>
            <input type="number" name="order_index" value="<?= $data['member']['order_index'] ?>" class="form-control">
        </div>

        <button type="submit" class="btn btn-primary w-100" style="height: 55px; font-weight: 700; font-size: 1.1rem;">Update Member Info</button>
    </form>
</div>
