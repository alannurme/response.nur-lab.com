    ...
    <form action="<?= URLROOT ?>/admin/edit_author/<?= $data['author']['id'] ?>" method="POST">
        ...
        <div class="form-group mb-4">
            ...
            <div style="display: flex; gap: 15px; align-items: flex-start;">
                <?php 
                    $hasImg = !empty($data['author']['image']);
                    $imgSrc = $hasImg ? htmlspecialchars($data['author']['image']) : '';
                ?>
                <img id="author_preview" src="<?= $imgSrc ?>" style="max-height: 80px; border-radius: 50%; display: <?= $hasImg ? 'block' : 'none' ?>; border: 1px solid #ddd;" loading="lazy" decoding="async">
                <div style="flex: 1;">
                    <input type="text" id="author_image_path" name="image" value="<?= $imgSrc ?>" class="form-control" readonly placeholder="Select image from media library">
