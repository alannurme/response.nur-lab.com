...
class Admin extends Controller {
    ...
    public function add_author() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($name)) {
                $_SESSION['author_error'] = 'Author name is required.';
                header('Location: ' . URLROOT . '/admin/authors');
                exit;
            }

            if (!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $_SESSION['author_error'] = 'Please enter a valid email address.';
                header('Location: ' . URLROOT . '/admin/authors');
                exit;
            }
            ...
            if ($adminModel->addAuthor($data)) {
                $_SESSION['author_success'] = 'Author added successfully!';
            } else {
                $_SESSION['author_error'] = 'Failed to add author.';
            }
            header('Location: ' . URLROOT . '/admin/authors');
            exit;
        }
    ...
    public function edit_author($id) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($name)) {
                $_SESSION['author_error'] = 'Author name cannot be empty.';
                header('Location: ' . URLROOT . '/admin/edit_author/' . $id);
                exit;
            }
            ...
            if ($adminModel->updateAuthor($id, $data)) {
                $_SESSION['author_success'] = 'Author updated successfully!';
                header('Location: ' . URLROOT . '/admin/authors');
                exit;
            } else {
                $_SESSION['author_error'] = 'Failed to update author.';
            }
        }
    ...
    public function delete_author($id) {
        $adminModel = $this->model('AdminModel');
        $author = $adminModel->getAuthorById($id);
        if ($author && $author['name'] === 'Admin') {
            $_SESSION['author_error'] = 'The primary Admin author cannot be deleted.';
        } else {
            if ($adminModel->deleteAuthor($id)) {
                $_SESSION['author_success'] = 'Author deleted safely. Linked articles were reassigned.';
            } else {
                $_SESSION['author_error'] = 'Failed to delete author.';
            }
        }
        header('Location: ' . URLROOT . '/admin/authors');
        exit;
    ...
    public function media() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_FILES['media_file'])) {
                ...
                if ($file['error'] !== UPLOAD_ERR_OK) {
                    ...
                    $forbidden_exts = ['php', 'phtml', 'php3', 'php4', 'php5', 'phps', 'phar', 'htaccess', 'htpasswd', 'pl', 'py', 'cgi', 'asp', 'aspx', 'exe', 'bat', 'cmd', 'sh', 'vbs'];
                    if (in_array($ext, $forbidden_exts, true)) {
                        if ($is_ajax) {
                            header('Content-Type: application/json');
                            echo json_encode(['success' => false, 'error' => 'Executable file type .' . $ext . ' is strictly forbidden for security reasons.']);
                            exit;
                        } else {
                            $data['error'] = 'Executable file type .' . $ext . ' is strictly forbidden!';
                            $file_name = null;
                        }
                    } else {
                        $file_name = time() . '_' . preg_replace('/[^a-zA-Z0-9_\.-]/', '_', $file['name']);
                    }
                    
                    if (!empty($file_name) && move_uploaded_file($file['tmp_name'], $upload_dir . $file_name)) {
                        if ($is_ajax) {
                            header('Content-Type: application/json');
        ...
        // Scan all public media directories recursively or by list
        $dirs_to_scan = [
            \PUBROOT . '/img',
            \PUBROOT . '/uploads/settings',
            \PUBROOT . '/uploads/products',
            \PUBROOT . '/uploads/pdf',
            \PUBROOT . '/uploads'
        ];
        ...
        foreach ($dirs_to_scan as $dir) {
            if (is_dir($dir)) {
                ...
                foreach ($items as $item) {
                    $path = $dir . '/' . $item;
                    if (is_file($path) && !isset($scanned_paths[$path])) {
                        $scanned_paths[$path] = true;
                        
                        // Calculate relative path from PUBROOT
                        $relPath = str_replace('\\', '/', substr($path, strlen(\PUBROOT)));
                        $url = URLROOT . '/public' . $relPath;
        ...
        if ($search !== '') {
            $all_files = array_filter($all_files, function($f) use ($search) {
                return mb_stripos($f['name'], $search) !== false;
            });
        }
    ...
    public function api_media() {
        ...
        // Limit the JSON response to prevent frontend rendering lag in modals (e.g. latest 200 items)
        $limit = 200;
    ...
    public function delete_media() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['file_path'])) {
            ...
            if ($realPath && $realPubroot && strpos(strtolower($realPath), strtolower($realPubroot)) === 0 && file_exists($realPath)) {
                if (unlink($realPath)) {
                    $_SESSION['media_success'] = 'Media asset deleted successfully!';
                } else {
                    $_SESSION['media_error'] = 'Failed to delete file from disk.';
                }
            } else {
                $_SESSION['media_error'] = 'Invalid file path or file does not exist.';
            }
        }
    ...
    public function settings() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['settings'])) {
                ...
                // Preserve existing SMTP password if left empty
    ...
    public function sync_islamic() {
        $action = $_GET['action'] ?? 'status';
        
        $db = \Config\Database::pdoConnect();
        
        // Auto-create tables on live server if not exist
        try {
            $db->exec("
            ...
            $db->exec("
                CREATE TABLE IF NOT EXISTS `hadith_cache` (
                    ...
                    `chapter_name` VARCHAR(255) CHARACTER SET utf8mb4 DEFAULT '',
                    `cached_at`    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                    `updated_at`   DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    UNIQUE KEY `uq_book_hadith` (`book_key`, `hadith_num`),
                    KEY `idx_book` (`book_key`),
        ...
        $syncedSurahs = [];
        try {
            $stmt = $db->query("SELECT COUNT(*) FROM quran_cache");
            $quranCount = (int)$stmt->fetchColumn();

            $stmtBnCount = $db->query("SELECT COUNT(*) FROM quran_cache WHERE TRIM(COALESCE(bangla, '')) != ''");
            $banglaCount = (int)$stmtBnCount->fetchColumn();

            $stmtTrCount = $db->query("SELECT COUNT(*) FROM quran_cache WHERE TRIM(COALESCE(transliteration, '')) != ''");
            $transliterationCount = (int)$stmtTrCount->fetchColumn();

            $stmtSurahs = $db->query("
                SELECT surah_num, surah_name_bn, surah_name_ar, total_ayahs, 
                       COUNT(*) as synced_count,
                       COUNT(NULLIF(TRIM(COALESCE(bangla, '')), '')) as bn_count,
                       COUNT(NULLIF(TRIM(COALESCE(transliteration, '')), '')) as tr_count,
                       COUNT(NULLIF(TRIM(COALESCE(tafsir_text, '')), '')) as tafsir_count,
                       MAX(tafsir_text) as sample_tafsir,
                       MAX(audio_url) as sample_audio,
                       GROUP_CONCAT(DISTINCT audio_url SEPARATOR '|||') as all_audios
                FROM quran_cache
                GROUP BY surah_num, surah_name_bn, surah_name_ar, total_ayahs
                ORDER BY surah_num ASC
            ");
            $syncedSurahs = $stmtSurahs->fetchAll();
        } catch(\Exception $e) {}

        // Count hadiths and compile books list
        $hadithCount = 0;
        $hadithBooksList = [];
        $masterHadithBooks = [
            'ben-bukhari'        => ['name' => 'সহীহ বুখারী', 'total' => 7529],
            'ben-muslim'         => ['name' => 'সহীহ মুসলিম', 'total' => 7360],
            'ben-abudawud'       => ['name' => 'সূনান আবু দাউদ', 'total' => 5270],
            'ben-tirmidhi'       => ['name' => 'জামে\' আত-তিরমিজী', 'total' => 3891],
            'ben-nasai'          => ['name' => 'সূনান আন-নাসায়ী', 'total' => 5656],
            'ben-ibnmajah'       => ['name' => 'সূনান ইবনে মাজাহ', 'total' => 4336],
            'ben-malik'          => ['name' => 'মুয়াত্তা মালিক', 'total' => 1749],
            'ben-nawawi'         => ['name' => '৪০ হাদিস (ইমাম নববী)', 'total' => 42],
        ];

        try {
            $stmt = $db->query("SELECT COUNT(*) FROM hadith_cache");
            $hadithCount = (int)$stmt->fetchColumn();

            $stmt2 = $db->query("
                SELECT book_key, 
                       COUNT(*) as synced_count,
                       COUNT(DISTINCT chapter_num) as chapter_count,
                       COUNT(NULLIF(TRIM(COALESCE(text_ar, '')), '')) as arabic_count
                FROM hadith_cache 
                GROUP BY book_key
            ");
            $syncedDbBooks = [];
            foreach ($stmt2->fetchAll() as $row) {
                $syncedDbBooks[$row['book_key']] = $row;
            }

            $syncedHadithBooksCount = 0;
            foreach ($masterHadithBooks as $key => $info) {
                $dbInfo = $syncedDbBooks[$key] ?? null;
                $sCnt = $dbInfo ? (int)$dbInfo['synced_count'] : 0;
                if ($sCnt > 0) $syncedHadithBooksCount++;
                ...
                $hadithBooksList[] = [
                    ...
                    'total'         => $info['total'],
                    'synced_count'  => $sCnt,
                    'chapter_count' => $dbInfo ? (int)$dbInfo['chapter_count'] : 0,
                    'arabic_count'  => $dbInfo ? (int)$dbInfo['arabic_count'] : 0,
                ];
            }
        } catch(\Exception $e) {}

        $data = [
            'title' => 'ইসলামিক ডাটা সিঙ্ক (Islamic Data Sync)',
            'settings' => $this->siteSettings,
            'current_page' => 'sync_islamic',
            'quranCount' => $quranCount,
            'banglaCount' => $banglaCount ?? $quranCount,
            'transliterationCount' => $transliterationCount ?? $quranCount,
            'syncedSurahs' => $syncedSurahs,
            'hadithCount' => $hadithCount,
            'hadithBooksList' => $hadithBooksList,
            'syncedHadithBooksCount' => $syncedHadithBooksCount ?? 0
        ];
        $this->view('admin/sync_islamic', $data);
    }
    ...
    public function add_page() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($title) && !empty($content)) {
                ...
                header('Location: ' . URLROOT . '/admin/pages?msg=created');
                exit;
            }
        }

        $this->view('admin/add_page', [
            'current_page' => 'pages',
            'settings' => $this->siteSettings
        ]);
    }
    ...
    public function edit_page($id) {
        $adminModel = $this->model('AdminModel');
        $page = $adminModel->getPageById($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($title) && !empty($content)) {
                $adminModel->updatePage($id, [
                    'title' => $title,
                    'content' => $content
                ]);
                header('Location: ' . URLROOT . '/admin/pages?msg=updated');
                exit;
            }
        }

        $this->view('admin/edit_page', [
            'page' => $page,
            'current_page' => 'pages',
            'settings' => $this->siteSettings
        ]);
    ...
    public function delete_page($id) {
        ...
        header('Location: ' . URLROOT . '/admin/pages?msg=deleted');
        exit;
    }
}
